...
author: "Thomas Scheuchzer <thomas.scheuchzer@zuehlke.com>"
...
Web page with 100 hits per day, standard deviation of 10. What's the probability that a given day we have fewer than 93 hits.

Convert 93 to standard normal quantile: $\frac{93 - 100}{10}$
...
### Conversion from standard normal $Z$ to normal $X$
...
```
# don't forget to use sigma and not sigma^2!
pnorm(x, mean = mu, sd = sigma, lower.tail = FALSE)
#or
1 - pnorm(x, mean = mu, sd = sigma)
```

or convert $x$ to standard normal deviations and read it from the standard normal graph as the percentiles are
well known.
...
$s$ for Poission(4) distribution is $2$.
