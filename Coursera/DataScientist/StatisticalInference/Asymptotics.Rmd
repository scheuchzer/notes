...
[Video](https://www.youtube.com/watch?feature=player_detailpage&list=UUKs7JbggJzbNEIw0t-PaiQA&v=dqgZxu9qc2w)

You sample 83 students and find that 63 regularly copy their homework. Make the 97% interval.

```
CI = statistic +- critical value * sd
```
$$
\hat p \pm Z^\ast \sqrt{ \frac{\hat p \hat q}{n} }
$$
...
ph <- round(63 / n, 3)
qh <- 1 - ph
Zcrit <- round(qnorm( (1-.97)/2, lower.tail = FALSE), 2)
```  
...
$$
`r ph` \pm `r Zcrit` \sqrt{ \frac{`r ph` * `r qh`}{`r n`}}
$$
```{r}
s <- round(sqrt(ph * qh / n), 4)
```  
$$
`r ph` \pm `r Zcrit` * `r s`
$$
